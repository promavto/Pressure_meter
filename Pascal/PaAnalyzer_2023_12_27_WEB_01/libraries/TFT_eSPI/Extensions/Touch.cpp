...
uint8_t TFT_eSPI::getTouchRaw(uint16_t *x, uint16_t *y){
 ...
 // spi.transfer16(0x91);                    // Start new YP conversion
 ...
 //// *y = data[3];// , data[3], data[5]);
  ...
  {*/
      ...
      *x = tmp;
      ...
      tmp = spi.transfer(0);                 // Read first 8 bits
      tmp = tmp << 5;
      tmp |= 0x1f & (spi.transfer(90) >> 3);    // Read last 8 bits
...
** Имя функции: validTouch
** Описание: читать проверенную позицию. Вернуть false, если не нажата.
***************************************************************************************/
#define _RAWERR 20 // Deadband error allowed in successive position samples Ошибка зоны нечувствительности допускается в последовательных выборках положения
uint8_t TFT_eSPI::validTouch(uint16_t *x, uint16_t *y, uint16_t threshold){
  uint16_t x_tmp, y_tmp, x_tmp2, y_tmp2;

  // Wait until pressure stops increasing to debounce pressure
 // Подождите, пока давление перестанет увеличиваться, чтобы устранить дребезг давления
  uint16_t z1 = 1;
  uint16_t z2 = 0;
  ...
  //  Serial.print("Z = ");Serial.println(z1);
   // Serial.print("IRQ = ");Serial.println(digitalRead(13));
...
void TFT_eSPI::calibrateTouch(uint16_t *parameters, uint32_t color_fg, uint32_t color_bg, uint8_t size){
  ...
  for(uint8_t i = 0; i<4; i++)
  {
	...
	{
      // Use a lower detect threshold as corners tend to be less sensitive  Используйте более низкий порог обнаружения, так как углы обычно менее чувствительны

...
#define BUTTON_HAS_INACTIVE_FONT_COLOR			8	
...
typedef struct
{
  ...
  uint16_t backColor, fontColor, inactiveFontColor;
  uint16_t			flags;
  const char				*label;
  FONTTYPE font;
...
class TFT_Buttons_Rus
{
   ...
   void setButtonFont(int buttonID, FONTTYPE font);
    ...
    word getButtonInactiveFontColor(int buttonID);
    void  setButtonInactiveFontColor(int buttonID, word color);

...
void RTC_DS3231::adjust(const DateTime &dt) 
{
	...
	RTCWireBus->_I2C_WRITE(bin2bcd(dowToDS3231(dt.dayOfWeek())));
...
bool RTC_DS3231::setAlarm1(const DateTime &dt, Ds3231Alarm1Mode alarm_mode) 
{
	...
	if (DY_DT) {
		RTCWireBus->_I2C_WRITE(bin2bcd(dowToDS3231(dt.dayOfWeek())) | A1M4 |
			DY_DT);
	}
	else {
...
bool RTC_DS3231::setAlarm2(const DateTime &dt, Ds3231Alarm2Mode alarm_mode) 
{
	...
	if (DY_DT) {
		RTCWireBus->_I2C_WRITE(bin2bcd(dowToDS3231(dt.dayOfWeek())) | A2M4 |
			DY_DT);
	}
	else {
...
const char* RTC_DS3231::getDayOfWeekStr(const RTCTime& t)
{
...
const char* RTC_DS3231::getTimeStr(const RTCTime& t)
{
...
//--------------------------------------------------------------------------------------------------------------------------------------
const char* RTC_DS3231::getDateStr(const RTCTime& t)
{
	char* writePtr = workBuff;
	if (t.dayOfMonth < 10)
		*writePtr++ = '0';
	...
	*writePtr++ = '.';
	...
	/*Вывожу только год в виде двух последних цифр*/
	//*writePtr++ = ((t.year - 2000) / 1000) + '0';
	//*writePtr++ = ((t.year - 2000) % 1000) / 100 + '0';
	*writePtr++ = ((t.year - 2000) % 100) / 10 + '0';
	*writePtr++ = ((t.year - 2000) % 10) + '0';

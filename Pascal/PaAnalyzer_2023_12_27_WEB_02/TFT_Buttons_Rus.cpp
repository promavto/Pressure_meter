...
TFT_Buttons_Rus::TFT_Buttons_Rus(TFT_Class *ptrUTFT, TOUCH_Class *ptrURTouch, TFTRus* pTFTRus, int count_btns)
{
  ...
  buttons = new button_type[count_btns];
	...
	_color_text_inactive	= TFT_LIGHTGREY;
...
//--------------------------------------------------------------------------------------------------------------------------------------
word TFT_Buttons_Rus::getButtonInactiveFontColor(int buttonID)
{
   if(buttonID < 0)
    return 0;

  return buttons[buttonID].inactiveFontColor;
}
//--------------------------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------------------------
void TFT_Buttons_Rus::setButtonInactiveFontColor(int buttonID, word cl)
{
  if(buttonID < 0 || buttonID >= countButtons)
  {
    return;
  ...
  buttons[buttonID].inactiveFontColor = cl;
  buttons[buttonID].flags |= BUTTON_HAS_INACTIVE_FONT_COLOR;
...
//Serial.println("4"); Serial.flush();
    ...
    {
      if (buttons[buttonID].flags & BUTTON_HAS_INACTIVE_FONT_COLOR)
      {
       textColor = buttons[buttonID].inactiveFontColor;
      }
      else
      {
...
int TFT_Buttons_Rus::checkButtons(OnCheckButtonsFunc pressed, OnCheckButtonsFunc released)
{
    ...
    if (_URTouch->getTouch(&touch_x, &touch_y))
    {
   ...
   // Serial.print(", y: "); Serial.println(touch_y);
		...
		if (result != -1)
		{    
      while (_URTouch->getTouch(&touch_x, &touch_y))
      {
        Ticker.tick(); 
        yield(); 
      }
